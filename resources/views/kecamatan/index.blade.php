...
<div class="x_panel">
    ...
    <div class="x_content">
        ...
        <a href="/kecamatan/create" class="btn btn-success"><i class="fa fa-plus"></i> TAMBAH KECAMATAN</a>
        <hr>
        <table class="table table-striped table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><a href="/kecamatan/{{ $k->id }}">{{ $k->nama }}</a></td>
                    <td>{{ $k->kode }}</td>
                    <td><a href="/kelurahan?q={{ $k->nama }}">{{ $k->kelurahan->count() }}</a></td>
                    <td><a href="/atm?q={{ $k->nama }}">{{ $k->atm->count() }}</a></td>
                    <td><a href="/penerima?q={{ $k->nama }}">{{ number_format($k->penerima->count()) }}</a></td>
                    <td class="text-right">
                        <div class="btn-group">
                            <a href="/kecamatan/{{$k->id}}/edit" class="btn btn-sm btn-default"><i class="fa fa-edit"></i></a>
                        ...
                        {!! Form::open(['method' => 'DELETE', 'url' => '/kecamatan/'.$k->id, 'style' => 'display:none;', 'id' => 'delete-kecamatan-'.$k->id]) !!}
